...
import { checkPromptSafety } from "./utils/safetyChecker.ts";
...
export async function callVertexAIGemini(
    ...
    try {
        ...
        // Check prompt safety before sending to Vertex AI
        log("Running safety check on prompt and reference images...");
        const safetyResult = await checkPromptSafety(prompt, safeParams.image);
        
        if (!safetyResult.safe) {
            // Use the safety checker's reason directly as the user-facing error message
            const safetyError = new Error(safetyResult.reason || 'Content safety violation detected');
            errorLog(`Safety check blocked request: ${safetyResult.reason}`);
            
            // Log as content policy violation
            await logNanoBananaError(prompt, safeParams, userInfo, safetyError);
            userStatsTracker.recordViolation(userInfo?.username);
            
            throw safetyError;
        }
        
        log("Safety check passed, proceeding with generation");
